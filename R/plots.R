...
plot_template <- function(data, x, y,
  ...
  if(!is.null(annotation_df)) {
    for(i in 1:length(annotation_df$x)) {
      plot <- plot + ggplot2::annotate("text", x=annotation_df$x[i], y=annotation_df$y[i], label=annotation_df$label[i])
    }
  }
}

career_plot <- function(name, miniY, stat) {
  data <- miniY %>% dplyr::filter(Punter==name)

  plot_template(data=data, x=year, y=stat,
                label=year, repel_size=1, repel_padding=1,
                point_size=NULL, point_alpha=NULL,
                image=url, image_size=0.05,
                yintercept=100, xintercept=100, intercept=NULL, slope=NULL,
                title = glue::glue('Career ', stat, 'for ', name),
                subtitle = NULL, xlabel = 'Year', ylabel = glue::glue(stat),
                caption = 'data from nflfastR | figure by @thePuntRunts',
                annotation_df = NULL)
